...
class ReadarrAPI extends ServarrBase<{ bookId: number }> {
  ...
  public getMetadataProfiles = async (): Promise<MetadataProfile[]> => {
    try {
      ...
      throw new Error(
        `[Readarr] Failed to retrieve metadata profiles: ${message}`
      );
    }
  };
  ...
  public getBooks = async (): Promise<ReadarrBook[]> => {
    try {
      ...
      const message = e instanceof Error ? e.message : String(e);
      throw new Error(`[Readarr] Failed to retrieve books: ${message}`);
    }
  };
  ...
  public getBook = async (id: number): Promise<ReadarrBook> => {
    try {
      ...
      const message = e instanceof Error ? e.message : String(e);
      throw new Error(`[Readarr] Failed to retrieve book: ${message}`);
    }
  };
  ...
  public async searchBooks(term: string): Promise<any[]> {
    try {
      const response = await this.axios.get<any[]>('/search', {
        params: { term },
      });
      const results = Array.isArray(response.data) ? response.data : [];
      // Filter out non-book results (e.g., author hits)
      return results.filter((item: any) => !!item?.book);
    } catch (e) {
  ...
  public removeBook = async (bookId: number): Promise<void> => {
    try {
      ...
      const message = e instanceof Error ? e.message : String(e);
      throw new Error(`[Readarr] Failed to remove book: ${message}`);
    }
  };
